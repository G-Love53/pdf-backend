...
console.log('COMPLETE FDF CONTENT:');
console.log(fdfData);  // This will show the ENTIRE FDF, not just first 500 chars
console.log('=== END FDF CONTENT ===');
    
    await fs.writeFile(fdfPath, fdfData);
    // Save debug copy to examine the full FDF
    await fs.writeFile('/tmp/debug-fdf.txt', fdfData);
    console.log('DEBUG: Full FDF saved to /tmp/debug-fdf.txt');
...
app.post('/fill-multiple', validateApiKey, async (req, res) => {
    try {
        ...
        // Create temp directory in project folder instead of /tmp
        const tempRoot = path.join(__dirname, 'temp');
        await fs.mkdir(tempRoot, { recursive: true });
        const tempDir = await fs.mkdtemp(path.join(tempRoot, 'pdf-filler-'));
...
app.post('/submit-quote', validateApiKey, async (req, res) => {
    try {
        ...
        const tempRoot = path.join(__dirname, 'temp');
        await fs.mkdir(tempRoot, { recursive: true });  // ADD THIS LINE
        const tempDir = await fs.mkdtemp(path.join(tempRoot, 'pdf-filler-'));
